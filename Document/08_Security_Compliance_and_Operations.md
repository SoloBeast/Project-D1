...
Authorization is permission-based (`AuthorizationCodes` policy checks on every endpoint); role names are never compared for capability decisions. JWT claims carry `role`, `permission`, and `branch_id`.

### Owner
Highest authority. Holds all 54 permissions, including the Owner-only capability `IDENTITY.ADMINISTRATORS.MANAGE`, which is required to create or manage System Administrators. The Owner may also manage all employees (`EMPLOYEES.READ` / `EMPLOYEES.MANAGE`) and has global branch scope.

### System Administrator
High-privilege operational admin. Holds 32 permissions covering employee management (`EMPLOYEES.READ`, `EMPLOYEES.MANAGE`) and global technical/operational administration, but explicitly does **not** hold Owner-only capabilities:

- No `IDENTITY.ADMINISTRATORS.MANAGE` — cannot create or manage other System Administrators.
- No role/permission administration permissions (`ROLES.READ` / `ROLES.MANAGE`) — cannot modify the RBAC model.
- No global branch scope beyond what `ACCESS.GLOBAL` grants for operational modules; Owner-only administration surfaces are not exposed.

System Administrators can create, invite, edit, deactivate, and reactivate employees for the assignable roles (Delivery Staff, Delivery Manager, Accountant, Dairy Manager, System Administrator), but assigning the `SYSTEM_ADMIN` role is enforced server-side to require `IDENTITY.ADMINISTRATORS.MANAGE`, so a System Administrator cannot create another System Administrator. The `OWNER` role is never assignable through employee management.
...
Employee lifecycle actions are also mandatory audited actions (`EMPLOYEE.CREATED`, `EMPLOYEE.INVITED`, `EMPLOYEE.REGISTERED`, `EMPLOYEE.ACTIVATED`, `EMPLOYEE.ROLE_CHANGED`, `EMPLOYEE.BRANCH_CHANGED`, `EMPLOYEE.DEACTIVATED`, `EMPLOYEE.REACTIVATED`, `EMPLOYEE.INVITATION_RESENT`, `EMPLOYEE.INVITATION_CANCELLED`). Each entry records the real authenticated actor's `user_id` — never a generic "System" actor — plus the snapshot (old/new), timestamp, entity, and action.

### Employee invitation security
- Tokens are generated by `SecureTokenGenerator` with cryptographic randomness; only the SHA-256 hash of the token is stored, and the raw token is returned to the caller exactly once.
- Tokens are encoded as unpadded base64url (RFC 4648 §5) so they survive URL-path routing verbatim — the invitee-facing link is `GET /api/v1/employee-invitations/{token}/verify`. Standard base64 (`+`, `/`, `=`) is deliberately not used because ASP.NET Core routing does not round-trip `%2F` inside a path segment.
- Tokens are single-use; verification, resend, and completion invalidate or replace them.
- Tokens expire after the configured lifetime (default 7 days; `invitationExpiresAt` may shorten it).
- Each token is bound to the invited employee's mobile/email; only that employee can complete registration.
- Role and branch are carried by the backend-issued invitation and applied server-side at completion. The client cannot escalate or change them.
- `OWNER` is never an assignable employee role; assigning `SYSTEM_ADMIN` requires the actor to hold `IDENTITY.ADMINISTRATORS.MANAGE` (Owner-only), enforced in `EnsureRoleAssignableAsync`.
- Registration OTP uses `OtpPurpose.EmployeeInvitation` (`3`), distinct from customer login/registration purposes.
