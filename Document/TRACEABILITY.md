...
| Bounded subscription generation | Delivery Manager generation is subscription-only and rejects dates outside the configurable inclusive operational window before mutation; duplicate occurrence delivery creation is prevented | `Backend/src/DoodhDirect.Infrastructure/Deliveries/DeliveryOptions.cs`; `Backend/src/DoodhDirect.Infrastructure/Deliveries/DeliveryService.cs`; `Backend/src/DoodhDirect.Api/Controllers/DeliveriesController.cs`; `Backend/tests/DoodhDirect.Api.IntegrationTests/DeliveryServiceTests.cs` |
| Delivery Manager filtering | Branch queue supports date, status, one-time/subscription source, and Morning/Evening subscription-slot filters while preserving branch/global authorization | `Backend/src/DoodhDirect.Application/Deliveries/DeliveryContracts.cs`; `Backend/src/DoodhDirect.Infrastructure/Deliveries/DeliveryService.cs`; `mobile/lib/features/deliveries/delivery_repository.dart`; `mobile/lib/features/deliveries/delivery_screens.dart` |
