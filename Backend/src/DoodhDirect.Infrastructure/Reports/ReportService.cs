...
public sealed class ReportService(DoodhDirectDbContext db, IClock clock) : IReportService
{
    ...
    public async Task<DashboardMetrics> GetDashboardAsync(ReportActor actor, DashboardRequest request, CancellationToken cancellationToken)
    {
        ...
        var from = request.DateRange?.FromUtc;
        var to = request.DateRange?.ToUtc;
        ...
        if (from.HasValue) { orders = orders.Where(x => x.CreatedAtUtc >= from); subscriptions = subscriptions.Where(x => x.CreatedAtUtc >= from); payments = payments.Where(x => x.CreatedAtUtc >= from); deliveries = deliveries.Where(x => x.CreatedAtUtc >= from); production = production.Where(x => x.ProductionAtUtc >= from); usage = usage.Where(x => x.UsedAtUtc >= from); tests = tests.Where(x => x.RequestedAtUtc >= from); events = events.Where(x => x.OccurredAtUtc >= from); }
        if (to.HasValue) { orders = orders.Where(x => x.CreatedAtUtc <= to); subscriptions = subscriptions.Where(x => x.CreatedAtUtc <= to); payments = payments.Where(x => x.CreatedAtUtc <= to); deliveries = deliveries.Where(x => x.CreatedAtUtc <= to); production = production.Where(x => x.ProductionAtUtc <= to); usage = usage.Where(x => x.UsedAtUtc <= to); tests = tests.Where(x => x.RequestedAtUtc <= to); events = events.Where(x => x.OccurredAtUtc <= to); }
    ...
    public async Task<ExportResult> ExportAsync(ReportActor actor, ExportRequest request, CancellationToken cancellationToken)
    {
        ...
        return new ExportResult(
            $"{request.Module.ToString().ToLowerInvariant()}-{clock.UtcNow:yyyyMMddHHmmss}.{extension}",
            contentType,
            content,
            rows.Length);
    }
    ...
    private async Task<ReportPage<OrderReportRow>> PageOrders(ReportActor a, ReportFilter f, CancellationToken ct)
    {
        ...
        if (f.DateRange?.FromUtc is { } from) q = q.Where(x => x.CreatedAtUtc >= from); if (f.DateRange?.ToUtc is { } to) q = q.Where(x => x.CreatedAtUtc <= to);
        ...
        var total = await q.CountAsync(ct); var data = await ApplyOrderSort(q, f).Skip((f.Page - 1) * f.PageSize).Take(f.PageSize).Select(x => new OrderReportRow(x.PublicId, x.OrderNumber, x.Type, x.Status, x.CreatedAtUtc, x.Customer.PublicId, x.Customer.DisplayName, x.Branch.PublicId, x.Branch.Code, x.Branch.Name, x.PayableAmount)).ToListAsync(ct); return MakePage(data, f, total);
    }
    ...
    private async Task<ReportPage<SubscriptionReportRow>> PageSubscriptions(ReportActor a, ReportFilter f, CancellationToken ct)
    {
        ...
        if (f.CustomerId.HasValue) q = q.Where(x => x.Customer.PublicId == f.CustomerId); if (f.ProductId.HasValue) q = q.Where(x => x.Product.PublicId == f.ProductId); var statuses = ParseStatuses<SubscriptionStatus>(f.Statuses); if (statuses.Count > 0) q = q.Where(x => statuses.Contains(x.Status)); if (f.DateRange?.FromUtc is { } from) q = q.Where(x => x.CreatedAtUtc >= from); if (f.DateRange?.ToUtc is { } to) q = q.Where(x => x.CreatedAtUtc <= to); if (!string.IsNullOrWhiteSpace(f.Search)) q = q.Where(x => (x.Customer.DisplayName ?? "").Contains(f.Search) || x.ProductNameSnapshot.Contains(f.Search));
    ...
    private async Task<ReportPage<DairyReportRow>> PageDairy(ReportActor a, ReportFilter f, CancellationToken ct)
    {
        ...
        var production = ApplyBranches(db.MilkProductions.AsNoTracking(), branches)
            .Select(x => new
            {
                ...
                OccurredAtUtc = x.ProductionAtUtc,
        ...
        var usage = ApplyBranches(db.MilkUsages.AsNoTracking(), branches)
            .Select(x => new
            {
                BranchId = db.Branches.Where(b => b.Id == x.BranchId).Select(b => b.PublicId).First(),
                BranchName = db.Branches.Where(b => b.Id == x.BranchId).Select(b => b.Name).First(),
                OccurredAtUtc = x.UsedAtUtc,
                Quantity = x.QuantityUsed,
                x.Batch.Unit,
                Purpose = (string?)x.Purpose
            });
        var query = production.Concat(usage);
        if (f.DateRange?.FromUtc is { } from) query = query.Where(x => x.OccurredAtUtc >= from);
        if (f.DateRange?.ToUtc is { } to) query = query.Where(x => x.OccurredAtUtc <= to);
        var total = await query.CountAsync(ct);
        var sorted = (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch
        {
            (null, _) or ("occurredatutc", ReportSortDirection.Descending) => query.OrderByDescending(x => x.OccurredAtUtc).ThenByDescending(x => x.BranchId),
            ("occurredatutc", ReportSortDirection.Ascending) => query.OrderBy(x => x.OccurredAtUtc).ThenBy(x => x.BranchId),
            ("quantity", ReportSortDirection.Ascending) => query.OrderBy(x => x.Quantity).ThenBy(x => x.OccurredAtUtc),
            ("quantity", ReportSortDirection.Descending) => query.OrderByDescending(x => x.Quantity).ThenByDescending(x => x.OccurredAtUtc),
            ("branchname", ReportSortDirection.Ascending) => query.OrderBy(x => x.BranchName).ThenBy(x => x.OccurredAtUtc),
            ("branchname", ReportSortDirection.Descending) => query.OrderByDescending(x => x.BranchName).ThenByDescending(x => x.OccurredAtUtc),
    ...
    {
        var q = ApplyBranches(db.MilkTests.AsNoTracking(), await ResolveBranchesAsync(a, f.BranchIds, ct)); if (f.CustomerId.HasValue) q = q.Where(x => x.Customer.PublicId == f.CustomerId); var statuses = ParseStatuses<MilkTestStatus>(f.Statuses); if (statuses.Count > 0) q = q.Where(x => statuses.Contains(x.Status)); if (f.DateRange?.FromUtc is { } from) q = q.Where(x => x.RequestedAtUtc >= from); if (f.DateRange?.ToUtc is { } to) q = q.Where(x => x.RequestedAtUtc <= to); var total = await q.CountAsync(ct); var data = await ApplyMilkTestSort(q, f).Skip((f.Page - 1) * f.PageSize).Take(f.PageSize).Select(x => new MilkTestReportRow(x.PublicId, db.Branches.Where(b => b.Id == x.BranchId).Select(b => b.PublicId).First(), db.Branches.Where(b => b.Id == x.BranchId).Select(b => b.Name).First(), x.Customer.PublicId, x.RequestedAtUtc, x.Status, x.CustomerDecision, x.Parameters.Count(), x.Images.Count(), x.CompletedAtUtc)).ToListAsync(ct); return MakePage(data, f, total);
    }
    ...
    {
        var q = ApplyNotificationScope(db.NotificationEvents.AsNoTracking(), a, await ResolveBranchesAsync(a, f.BranchIds, ct)); var statuses = ParseStatuses<NotificationEventStatus>(f.Statuses); if (statuses.Count > 0) q = q.Where(x => statuses.Contains(x.Status)); if (f.DateRange?.FromUtc is { } from) q = q.Where(x => x.OccurredAtUtc >= from); if (f.DateRange?.ToUtc is { } to) q = q.Where(x => x.OccurredAtUtc <= to); if (!string.IsNullOrWhiteSpace(f.Search)) q = q.Where(x => x.EventType.Contains(f.Search)); var total = await q.CountAsync(ct); var data = await ApplyNotificationSort(q, f).Skip((f.Page - 1) * f.PageSize).Take(f.PageSize).Select(x => new NotificationReportRow(x.PublicId, x.EventType, x.Status, x.OccurredAtUtc, x.IsCritical, x.Notifications.SelectMany(n => n.Deliveries).Count(), x.Notifications.SelectMany(n => n.Deliveries).Count(d => d.Status == NotificationDeliveryStatus.Failed), x.Notifications.SelectMany(n => n.Deliveries).Sum(d => d.AttemptCount))).ToListAsync(ct); return MakePage(data, f, total);
    }

    private async Task<ReportPage<AuditReportRow>> PageAudit(ReportActor a, ReportFilter f, CancellationToken ct)
    {
        var q = db.AuditLogs.AsNoTracking(); if (!a.HasGlobalAccess) q = q.Where(x => x.UserId == a.UserId || (x.UserId.HasValue && db.UserRoles.Any(r => r.UserId == x.UserId && r.BranchId.HasValue && a.BranchIds.Contains(r.BranchId.Value)))); if (f.DateRange?.FromUtc is { } from) q = q.Where(x => x.CreatedAtUtc >= from); if (f.DateRange?.ToUtc is { } to) q = q.Where(x => x.CreatedAtUtc <= to); if (!string.IsNullOrWhiteSpace(f.Search)) q = q.Where(x => x.Action.Contains(f.Search) || x.EntityType.Contains(f.Search)); var total = await q.CountAsync(ct); var data = await ApplyAuditSort(q, f).Skip((f.Page - 1) * f.PageSize).Take(f.PageSize).Select(x => new AuditReportRow(x.Id, x.UserId, x.Action, x.EntityType, x.EntityId, x.Reason, x.CreatedAtUtc)).ToListAsync(ct); return MakePage(data, f, total);
    }
    ...
    private static IQueryable<User> ApplyCustomerSort(IQueryable<User> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("createdatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CreatedAtUtc).ThenByDescending(x => x.Id), ("createdatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CreatedAtUtc).ThenBy(x => x.Id), ("name" or "displayname", ReportSortDirection.Ascending) => q.OrderBy(x => x.DisplayName).ThenBy(x => x.Id), ("name" or "displayname", ReportSortDirection.Descending) => q.OrderByDescending(x => x.DisplayName).ThenByDescending(x => x.Id), ("isactive", ReportSortDirection.Ascending) => q.OrderBy(x => x.IsActive).ThenBy(x => x.Id), ("isactive", ReportSortDirection.Descending) => q.OrderByDescending(x => x.IsActive).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Customers) };
    private static IQueryable<User> ApplyEmployeeSort(IQueryable<User> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("createdatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CreatedAtUtc).ThenByDescending(x => x.Id), ("createdatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CreatedAtUtc).ThenBy(x => x.Id), ("name" or "displayname", ReportSortDirection.Ascending) => q.OrderBy(x => x.DisplayName).ThenBy(x => x.Id), ("name" or "displayname", ReportSortDirection.Descending) => q.OrderByDescending(x => x.DisplayName).ThenByDescending(x => x.Id), ("isactive", ReportSortDirection.Ascending) => q.OrderBy(x => x.IsActive).ThenBy(x => x.Id), ("isactive", ReportSortDirection.Descending) => q.OrderByDescending(x => x.IsActive).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Employees) };
    private static IQueryable<Order> ApplyOrderSort(IQueryable<Order> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("createdatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CreatedAtUtc).ThenByDescending(x => x.Id), ("createdatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CreatedAtUtc).ThenBy(x => x.Id), ("ordernumber", ReportSortDirection.Ascending) => q.OrderBy(x => x.OrderNumber).ThenBy(x => x.Id), ("ordernumber", ReportSortDirection.Descending) => q.OrderByDescending(x => x.OrderNumber).ThenByDescending(x => x.Id), ("status", ReportSortDirection.Ascending) => q.OrderBy(x => x.Status).ThenBy(x => x.Id), ("status", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Status).ThenByDescending(x => x.Id), ("payableamount", ReportSortDirection.Ascending) => q.OrderBy(x => x.PayableAmount).ThenBy(x => x.Id), ("payableamount", ReportSortDirection.Descending) => q.OrderByDescending(x => x.PayableAmount).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Orders) };
    private static IQueryable<Subscription> ApplySubscriptionSort(IQueryable<Subscription> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("createdatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CreatedAtUtc).ThenByDescending(x => x.Id), ("createdatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CreatedAtUtc).ThenBy(x => x.Id), ("startdate", ReportSortDirection.Ascending) => q.OrderBy(x => x.StartDate).ThenBy(x => x.Id), ("startdate", ReportSortDirection.Descending) => q.OrderByDescending(x => x.StartDate).ThenByDescending(x => x.Id), ("enddate", ReportSortDirection.Ascending) => q.OrderBy(x => x.EndDate).ThenBy(x => x.Id), ("enddate", ReportSortDirection.Descending) => q.OrderByDescending(x => x.EndDate).ThenByDescending(x => x.Id), ("status", ReportSortDirection.Ascending) => q.OrderBy(x => x.Status).ThenBy(x => x.Id), ("status", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Status).ThenByDescending(x => x.Id), ("payableamount", ReportSortDirection.Ascending) => q.OrderBy(x => x.PayableAmount).ThenBy(x => x.Id), ("payableamount", ReportSortDirection.Descending) => q.OrderByDescending(x => x.PayableAmount).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Subscriptions) };
    private static IQueryable<Payment> ApplyPaymentSort(IQueryable<Payment> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("createdatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CreatedAtUtc).ThenByDescending(x => x.Id), ("createdatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CreatedAtUtc).ThenBy(x => x.Id), ("amount", ReportSortDirection.Ascending) => q.OrderBy(x => x.Amount).ThenBy(x => x.Id), ("amount", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Amount).ThenByDescending(x => x.Id), ("refundedamount", ReportSortDirection.Ascending) => q.OrderBy(x => x.RefundedAmount).ThenBy(x => x.Id), ("refundedamount", ReportSortDirection.Descending) => q.OrderByDescending(x => x.RefundedAmount).ThenByDescending(x => x.Id), ("status", ReportSortDirection.Ascending) => q.OrderBy(x => x.Status).ThenBy(x => x.Id), ("status", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Status).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Payments) };
    ...
    private static IQueryable<Delivery> ApplyDeliverySort(IQueryable<Delivery> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("scheduleddate", ReportSortDirection.Descending) => q.OrderByDescending(x => x.ScheduledDate).ThenByDescending(x => x.Id), ("scheduleddate", ReportSortDirection.Ascending) => q.OrderBy(x => x.ScheduledDate).ThenBy(x => x.Id), ("status", ReportSortDirection.Ascending) => q.OrderBy(x => x.Status).ThenBy(x => x.Id), ("status", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Status).ThenByDescending(x => x.Id), ("completedatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CompletedAtUtc).ThenBy(x => x.Id), ("completedatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CompletedAtUtc).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.Deliveries) };
    private static IQueryable<MilkTest> ApplyMilkTestSort(IQueryable<MilkTest> q, ReportFilter f) => (f.SortBy?.ToLowerInvariant(), f.SortDirection) switch { (null, _) or ("requestedatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.RequestedAtUtc).ThenByDescending(x => x.Id), ("requestedatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.RequestedAtUtc).ThenBy(x => x.Id), ("status", ReportSortDirection.Ascending) => q.OrderBy(x => x.Status).ThenBy(x => x.Id), ("status", ReportSortDirection.Descending) => q.OrderByDescending(x => x.Status).ThenByDescending(x => x.Id), ("completedatutc", ReportSortDirection.Ascending) => q.OrderBy(x => x.CompletedAtUtc).ThenBy(x => x.Id), ("completedatutc", ReportSortDirection.Descending) => q.OrderByDescending(x => x.CompletedAtUtc).ThenByDescending(x => x.Id), _ => throw UnsupportedSort(f.SortBy!, ReportModule.MilkTests) };
    ...
    private static void ValidateDateRange(ReportDateRange? dateRange) { if (dateRange?.FromUtc > dateRange?.ToUtc) throw new ValidationAppException("The report date range is invalid.", nameof(ReportDateRange)); if (dateRange?.FromUtc is { Kind: not DateTimeKind.Utc } || dateRange?.ToUtc is { Kind: not DateTimeKind.Utc }) throw new ValidationAppException("Report dates must be UTC.", nameof(ReportDateRange)); }
